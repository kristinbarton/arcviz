...
def extract_trajectory(grid, track, mode, method='nearest'):
    ...
    if isinstance(grid, xr.DataArray):
        grid_daily = grid.resample(time='1D').mean()
    elif isinstance(grid.da, xr.Dataset):
        grid_daily = grid.da[grid.varname].resample(time='1D').mean()
    elif isinstance(grid.da, xr.DataArray):
        grid_daily = grid.da.resample(time='1D').mean()

    if track.synced_da is None: 
        track_sync = track.da.sel(time=grid_daily['time'], method='nearest')
    else:
        track_sync = track.synced_da

    lat_coord = grid_daily['lat']
    lon_coord = grid_daily['lon']

    # For 1D coordinates, xarray can handle the remapping
    if lat_coord.ndim == 1:
        if mode == 'moving':
            extracted = grid_daily.sel(
                lat=track_sync['lat'],
                lon=track_sync['lon'],
                method=method
            )
    ...
    # Otherwise, we will need to build a KDTree
        ...
        if mode == 'moving':
            ...
            extracted = grid_daily.isel(time=ntda, nj=njda, ni=nida,)
        elif mode == 'stationary':
            njda = xr.DataArray(idx_j, dims=['track_point'])
            nida = xr.DataArray(idx_i, dims=['track_point'])
            extracted = grid_daily.isel(nj=njda, ni=nida)
